...
- The on-device pass with VoiceOver, Voice Control and Switch Control cannot be automated. It is the *Accessibility (on device)* section of the [device test plan](DEVICE_TEST_PLAN.md), run on the TestFlight build of every release candidate and recorded in that release's issue. That plan is the only checklist: a second copy here would drift away from it.
