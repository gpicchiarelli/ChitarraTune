...
macOS 27 may refuse to launch an unsigned executable started from Xcode, so use a personal team if you have no paid account. Building from the command line without signing works, but the result is only good for compile checks:
