...
final class MetricsReporter: NSObject, MXMetricManagerSubscriber {
    ...
    /// Appends summaries to the stored ones and keeps the newest ``limit``.
    nonisolated static func store(_ summaries: [String], in defaults: UserDefaults = .standard) {
        guard !summaries.isEmpty else { return }
        storageLock.withLock { _ in
            let kept = (defaults.stringArray(forKey: storageKey) ?? []) + summaries
            defaults.set(Array(kept.suffix(limit)), forKey: storageKey)
        }
    }
