...
Thanks for helping. Issues and pull requests are welcome in English or Italian. Taking part means keeping to the [code of conduct](CODE_OF_CONDUCT.md).
...
You need Xcode 27 or later. Clone and build as the [README](README.md#build-from-source) shows, and put your own signing team in `Config/Local.xcconfig`, which git ignores:
...
Because changes go straight to `main` and the ruleset requires no status check, this hook is the gate in practice — GitHub reports, it does not refuse. CI repeats everything on GitHub (**Gate**, **Lint**, **CodeQL**) and adds what a laptop should not have to run: the iOS release build with pointer authentication, the UI tests on iPhone and iPad, the Mac compile, the release check with its disk image, and the optimized DSP matrix. A red run is fixed before anything else. Every run also prints what it cost — each job beside its entry in [`.github/ci-baseline.json`](.github/ci-baseline.json), and each test beside the others in its bundle — so a change that makes the gate slower says so on the run that makes it ([ADR 0020](docs/adr/0020-the-gate-reports-its-own-cost.md)). Those numbers warn and never fail: the runners are far too variable for a timing to be a verdict. `Scripts/test-timings.sh` reads the same thing from a result bundle on your own machine. The Mac *UI tests* are the one thing that cannot stop the gate, for a reason and with an end date: [RELEASE_READINESS.md](docs/RELEASE_READINESS.md) B6.
...
One check stays outside the gate: `Scripts/check-links.sh` asks every external link in the documentation whether it still answers. It needs the network, and a host that is down for ten minutes must not fail a commit that is otherwise correct, so it runs weekly in the **Links** workflow. Relative links are checked offline, in the gate, with everything else.
