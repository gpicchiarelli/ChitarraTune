...
final class AppStoreScreenshots: XCTestCase {
    ...
    /// One bound for every reading, and it only decides when a failing screenshot gives up: the
    /// expectation returns the moment the readout matches, so waiting longer costs nothing when it
    /// arrives. A simulator booted cold takes appreciably longer to settle than a warm one, and taking
    /// screenshots in the other language forces a cold boot, because the device's own language is read
    /// at boot. Whichever test ran first then failed — 20 seconds on the first reading, 30 on the
    /// pinned top string — while the same test passed in 77 seconds on a warm device.
    private func waitForReadout(matching pattern: String, timeout: TimeInterval = 90) {
