...
None of this is a promise you have to take on trust: each line is a [decision on the record](docs/adr/README.md), with a test that fails if the code drifts away from it. The policy is in [PRIVACY.md](PRIVACY.md); to report a vulnerability privately, see [SECURITY.md](SECURITY.md).
...
Issues and pull requests are welcome, in English or Italian. [CONTRIBUTING.md](CONTRIBUTING.md) lists the few rules that keep the app small, private and precise, and [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md) what is expected of everyone taking part. Bugs and ideas go through the [issue forms](https://github.com/gpicchiarelli/ChitarraTune/issues/new/choose); questions about using the app, to [SUPPORT.md](SUPPORT.md).
...
**Compilare.** Serve Xcode 27 o successivo. Apri `ChitarraTune.xcodeproj`, scegli lo schema **ChitarraTune** e una destinazione. Per firmare con il tuo team crea `Config/Local.xcconfig` con `DEVELOPMENT_TEAM = TUOTEAMID`. I test del motore non richiedono microfono: `swift test --package-path Packages/ChitarraTuneKit`.
...
<sub>Apple, il logo Apple, Mac, macOS, iOS, iPadOS, iPhone, iPad, Xcode, Siri e Comandi Rapidi sono marchi di Apple Inc., registrati negli Stati Uniti e in altri paesi e regioni. App Store e Mac App Store sono marchi di servizio di Apple Inc. Swift e il logo Swift sono marchi di Apple Inc. ChitarraTune è un progetto indipendente, non affiliato ad Apple né approvato da Apple.</sub>
