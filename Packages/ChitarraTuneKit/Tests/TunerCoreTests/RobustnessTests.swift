...
struct RobustnessTests {
    ...
    /// Recovering means measuring as well as before, not merely producing a number again. A non-finite
    /// sample used to stay in the string filters' state for ever: the low-partial refinement declined
    /// from then on and a plucked low E read 2.9 cents sharp — the inharmonicity the refinement exists
    /// to remove — while the display looked perfectly normal.
    @Test("A non-finite sample does not leave the measurement biased", arguments: [Float.nan, .infinity, -.infinity])
    func accuracyAfterNonFiniteSamples(value: Float) throws {
        let string = StringModel(roomNoise: 0.000_2, hum: 0.000_1, amplitude: 0.3)
            .pluck(frequency: Tuning.standard.strings[0].frequency(), sampleRate: 44_100, duration: 2)
        func medianError(poisoned: Bool) throws -> Double {
            var engine = TuningEngine()
            if poisoned { _ = feed(&engine, [Float](repeating: value, count: 8_192)) }
            let cents = feed(&engine, string).compactMap(\.reading).filter { !$0.isHeld }.map(\.cents).sorted()
            return try #require(cents.isEmpty ? nil : cents[cents.count / 2])
        }
        let clean = try medianError(poisoned: false)
        let afterwards = try medianError(poisoned: true)
        #expect(abs(afterwards - clean) < 0.1, "median \(clean) ¢ clean, \(afterwards) ¢ after \(value)")
    }
