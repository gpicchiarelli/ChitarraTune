...
# Keep a Changelog: every bracketed version needs a link definition, and Unreleased compares
# against the newest tag instead of the whole history.
REPO="https://github.com/gpicchiarelli/ChitarraTune"
sed -i '' -E "s|^\[Unreleased\]: .*|[Unreleased]: $REPO/compare/v$VERSION...HEAD|" CHANGELOG.md
sed -i '' -E "s|^\[Unreleased\]: (.*)\$|[Unreleased]: \\1\\
[$VERSION]: $REPO/releases/tag/v$VERSION|" CHANGELOG.md
