...
attach() {
  local try
  for try in 1 2 3; do
    hdiutil attach "$1" -nobrowse -readonly -noverify -noautoopen -mountpoint "$MOUNT" >/dev/null 2>&1 \
...
if [ -z "$VERIFY" ]; then
  ...
  hdiutil create -srcfolder "$STAGE" -volname "$VOLUME" \
    -fs HFS+ -format UDZO -imagekey zlib-level=9 -nospotlight -srcowners off -ov "$DMG" >/dev/null
  pass "created $DMG"
